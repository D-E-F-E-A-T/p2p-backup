...
func GetReserveList(c *gin.Context) {
	...
	count, deals, err := postgres.Postgres.GetReserveDeals(request.Limit, (request.Page-1)*request.Limit)
	if err != nil {
		if err == pg.ErrNoRows {
